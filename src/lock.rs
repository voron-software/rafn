...
impl BenchLock {
    ...
    fn acquire_at(path: &Path, timeout: Duration) -> Result<Self> {
        ...
        // `checked_add` rather than `+`: a timeout large enough to overflow
        // `Instant` yields `None`, which we treat as "wait indefinitely"
        // instead of panicking.
        let deadline = Instant::now().checked_add(timeout);
        loop {
            // Enforce the deadline *before* each attempt so the configured
            // timeout is a true upper bound — we never proceed once it has
            // elapsed, even if the lock happened to free up during the sleep.
            ...
            let nap = remaining.map_or(POLL_INTERVAL, |r| POLL_INTERVAL.min(r));
